    ...
    def train_model(self, model, train_loader, val_loader, lr, epochs, optimizers, patience, factor):
        ...
        for epoch in range(epochs):
            ...
            for inputs, y_true in train_loader:
                ...
                p_t = (11 * y_pred[:, :, 1] - 18 * inputs[:, -2, 1] + 9 * inputs[:, -3, 1] - 2 * inputs[:, 0, 1]) / (6 * self.dt)
                fLoss_mass = torch.mean(torch.square(m_t - (self.A1 / self.Lc) * ((interp * self.P1) - y_true[:, :, 1]) * 1e3))
                press_l = (self.C**2) / 2 * (y_true[:, :, 0] - inputs[:, -1, -2] * self.kv * (np.abs(y_true[:, :, 1] * 1e3 - self.P_out * 1e3))**(1/2))
                fLoss_pres = torch.mean(torch.square(p_t - press_l*1e-7))
